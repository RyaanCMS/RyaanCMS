                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                        ...
                        <form @submit.prevent x-data="{ preview: '{{ $savedLogo ? Storage::url($savedLogo) : '' }}', dragging: false, uploading: false,
                            ...
                            <div class="bk-upload-zone" :class="dragging ? 'bk-upload-drag' : ''"
                                ...
                                <template x-if="!preview">
                                    <div style="display:flex;flex-direction:column;align-items:center;gap:8px;">
                                        <div style="width:56px;height:56px;border-radius:12px;background:linear-gradient(135deg,#6366f1,#8b5cf6);display:flex;align-items:center;justify-content:center;font-size:22px;font-weight:800;color:#fff;letter-spacing:-1px;flex-shrink:0;">
                                            R
                                        </div>
                                        <div style="font-size:11.5px;font-weight:600;color:var(--text-2);">Default brand icon</div>
                                        <div style="font-size:10.5px;color:var(--text-3);">Click to upload custom logo</div>
                                    </div>
                                </template>
                            </div>
                        ...
                        <form @submit.prevent x-data="{ preview: '{{ $savedFav ? Storage::url($savedFav) : '' }}', dragging: false, uploading: false,
                            ...
                            <div class="bk-upload-zone" :class="dragging ? 'bk-upload-drag' : ''"
                                ...
                                <template x-if="!preview">
                                    <div style="display:flex;flex-direction:column;align-items:center;gap:8px;">
                                        <div style="width:48px;height:48px;border-radius:10px;border:1px solid var(--border);overflow:hidden;background:#fff;display:flex;align-items:center;justify-content:center;">
                                            <img src="{{ asset('favicon.svg') }}" style="width:32px;height:32px;object-fit:contain;" onerror="this.style.display='none'">
                                        </div>
                                        <div style="font-size:11.5px;font-weight:600;color:var(--text-2);">Default favicon</div>
                                        <div style="font-size:10.5px;color:var(--text-3);">Click to upload custom favicon</div>
                                    </div>
                                </template>
                            </div>
