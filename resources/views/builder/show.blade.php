...
<div class="flex h-full" x-data="builderApp()" x-init="init()">
    ...
    <div class="flex-shrink-0 flex flex-col" :style="`width:${chatPanelWidth}px; min-width:260px; max-width:640px; background:#fff; border-right:1px solid #e5e7eb;`">
        ...
        <!-- Smart Panels: Questions + Components (collapsed by default) -->
        <div x-data="{
            smartPanel: null,
            questions: [],
            components: [],
            componentSearch: '',
            loadQuestions() {
                fetch('{{ route('builder.smart_questions', $project) }}')
                    .then(r=>r.json())
                    .then(d=>{ this.questions = d.questions || []; });
            },
            loadComponents(q) {
                const url = '{{ route('builder.components') }}' + (q ? '?search='+encodeURIComponent(q) : '');
                fetch(url).then(r=>r.json()).then(d=>{ this.components = d.components || []; });
            },
            insertComponent(key, label) {
                fetch('{{ url('builder/'.$project->id.'/components') }}/' + key + '/insert', {
                    method: 'POST',
                    headers: {'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content, 'Content-Type':'application/json'}
                }).then(r=>r.json()).then(d=>{
                    if(d.success) {
                        this.$dispatch('component-inserted', { message: d.message, tokens_saved: d.tokens_saved });
                        this.smartPanel = null;
                    }
                });
            },
            useQuestion(q) {
                this.$dispatch('use-question', { text: q });
                this.smartPanel = null;
            }
        }"
        ...
        class="border-t" style="border-color:#e5e7eb; background:#fafafa;">
            ...
            <div class="flex items-center gap-1 px-3 py-2">
                <button @click="if(smartPanel==='questions'){smartPanel=null}else{smartPanel='questions';loadQuestions()}"
                        :class="smartPanel==='questions' ? 'bg-indigo-50 text-indigo-600 border-indigo-200' : 'text-gray-400 border-gray-200'"
                ...
                <button @click="if(smartPanel==='components'){smartPanel=null}else{smartPanel='components';loadComponents('')}"
                        :class="smartPanel==='components' ? 'bg-violet-50 text-violet-600 border-violet-200' : 'text-gray-400 border-gray-200'"
                        class="flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-xs font-medium transition-all">
                    <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div x-show="smartPanel === 'questions'" x-transition class="px-3 pb-3">
                <div class="rounded-xl border overflow-hidden" style="border-color:#e5e7eb;background:#fff">
                    <div class="px-3 py-2 border-b text-xs font-semibold" style="border-color:#e5e7eb;color:#4b5563;background:#f9fafb">
                        💡 Key questions for <span class="text-indigo-600">{{ $project->type ?? 'your project' }}</span> — click to use
                    </div>
                    <div class="divide-y" style="divide-color:#f3f4f6; max-height:200px; overflow-y:auto">
                        <template x-for="(q, i) in questions" :key="i">
                            <button @click="useQuestion(q)"
                                    class="w-full text-left px-3 py-2 text-xs transition-colors hover:bg-indigo-50"
                                    style="color:#374151">
                                <span class="text-indigo-400 mr-1">▸</span>
                                <span x-text="q"></span>
                            </button>
                        </template>
                        <div x-show="!questions.length" class="px-3 py-4 text-center text-xs" style="color:#9ca3af">
                            Loading questions…
                        </div>
                    </div>
                </div>
                        ...
                        <template x-for="c in components" :key="c.key">
                            <button @click="insertComponent(c.key, c.label)"
                                ...
                                <div class="flex items-start justify-between gap-1">
                                    <span class="text-xs font-semibold" style="color:#374151" x-text="c.label"></span>
                                    <span class="text-xs px-1 rounded flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity"
                                          style="background:#7c3aed20;color:#7c3aed" x-text="'−'+c.tokens_saved+'t'"></span>
                                </div>
                                <p class="text-xs mt-0.5 leading-relaxed" style="color:#9ca3af" x-text="c.description"></p>
                            </button>
                        </template>
                        <div x-show="!components.length" class="col-span-2 px-3 py-4 text-center text-xs" style="color:#9ca3af">
                            No components found.
                        </div>
                    </div>
                </div>
