...
TINY;
    ...
    private function generateBlueprintDriven(
        ...
        $entityNames = $autoCount > 0 ? implode(', ', $autoGenerated) : 'none';
        $displayMsg  = $parsed['message']
            ?: "Built with Blueprint-Driven approach: {$autoCount} modules auto-generated ({$entityNames}), {$aiCount} files from AI. Total cost: ~{$totalTokens} tokens.";
    ...
    private function streamBlueprintDriven(
        ...
        $onEvent(['type' => 'activity', 'icon' => '🗺️',
                  'text' => "Blueprint ready: {$blueprint['name']} ({$packNames})"]);
        ...
        foreach ($entities as $entity) {
            ...
            $onEvent(['type' => 'activity', 'icon' => '⚡',
                      'text' => "Auto-generating {$entityName} module (zero tokens)..."]);
            $files      = $this->crudGenerator->generate($entity);
            $savedBatch = [];
            foreach ($files as $f) {
                $saved = $this->saveFileToProject($project, $f['path'], $f['content']);
        ...
        $onEvent(['type' => 'activity', 'icon' => '🤖',
                  'text' => 'Building auth, dashboard & complex logic with AI...']);
...
Rule: Build only what the blueprint defines. Do not invent extra entities or features.
BP;
        }
