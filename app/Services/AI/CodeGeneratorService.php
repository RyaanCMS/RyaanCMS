...
SRS;
    ...
    private function generateBlueprintDriven(
        ...
        // ── Step 1: Library lookup first (zero AI) — AI only for unknown domains ──
        $blueprint = $this->blueprintService->tryLibrary($prompt);
        if (!$blueprint) {
            $candidates = $this->aiManager->providerFallbackCandidates($provider, $user);
            if (empty($candidates)) {
                throw new \RuntimeException('This looks like a custom domain. Add an AI provider in Settings → AI Providers to generate it. (95% of standard apps build without an AI key.)');
            }
        ...
        // ── Step 3: AI call for complex parts (skip if no provider configured) ──
        $candidates3 = $this->aiManager->providerFallbackCandidates($provider, $user);
        if (empty($candidates3)) {
            $fileCount  = count($allFiles);
            $noAiMsg    = "✅ **{$project->name} core modules ready** — {$fileCount} files built. Add an AI provider in **Settings → AI Providers** to generate the dashboard, navigation and advanced business logic.";
    ...
    private function streamBlueprintDriven(
        ...
        // Heartbeat-aware sleep — sends ping every 1s so SSE connection stays alive
        $sleep = function (int $seconds) use ($onEvent): void {
            for ($i = 0; $i < $seconds; $i++) {
                usleep(1000000);
                $onEvent(['type' => 'ping']);
            }
        ...
        // ── Step 1: Library lookup first (zero AI) — discover() only if no library match ──
        $onEvent(['type' => 'activity', 'icon' => '🔍', 'text' => 'Analyzing your request...']);
        $blueprint      = $this->blueprintService->tryLibrary($prompt);
        $isLibraryMatch = $blueprint !== null;
        if (!$blueprint) {
            if ($aiProvider === null) {
                $onEvent(['type' => 'error', 'message' => 'This looks like a custom domain. Add an AI provider in Settings → AI Providers to generate it. (95% of standard apps build without an AI key.)']);
                return;
            }
            $onEvent(['type' => 'activity', 'icon' => '🧠', 'text' => 'Understanding domain requirements...']);
            $blueprint = $this->blueprintService->discover($prompt, $aiProvider);
        }
        $this->blueprintService->store($project, $blueprint);
        $totalTokens += $blueprint['tokens_used'] ?? 0;

        if ($isLibraryMatch) { $sleep(5); } // 5s — analyzing phase
        ...
        foreach ($entities as $entity) {
            ...
            if ($isLibraryMatch) { $sleep(8); } // 8s per entity
            $files      = $this->crudGenerator->generate($entity);
            $savedBatch = [];
            foreach ($files as $f) {
                $saved = $this->saveFileToProject($project, $f['path'], $f['content']);
        ...
        if (!empty($allFiles)) {
            $onEvent(['type' => 'files', 'files' => array_slice($allFiles, -4)]);
        }

        // ── Step 3: Stream AI for complex parts (skipped if no provider) ────────
        if ($aiProvider === null) {
            if ($isLibraryMatch) {
...
HTML;
    ...
    public function streamGenerate(
        ...
        try {
            ...
            // Try to resolve AI provider — null means no key configured (zero-cost path only)
            $aiProvider = null;
